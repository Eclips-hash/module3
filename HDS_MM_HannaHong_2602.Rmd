...
This report examines whether IntClust classification reveals clinically meaningful heterogeneity within PAM50 Luminal breast cancer, focusing on three analytical questions. I first characterise gene expression patterns of therapeutically relevant genes (ESR1, CCND1, FGFR1) across IntClust groups. I then identify mutation profiles, particularly TP53 and PIK3CA, and assess how they differ across subtypes. Finally, I evaluate whether IntClust stratifies patients beyond PAM50 and standard clinical variables through survival analysis. As an extension, I quantify the proportion of PAM50 Luminal patients with hidden high-risk profiles and consider implications for how risk is communicated to patients.
...
I chose IntClust as the subtype definition rather than PAM50 because IntClust integrates copy number alterations with expression data, potentially revealing prognostic heterogeneity hidden within PAM50 categories (Curtis et al. 2012). For gene expression analysis, I focused on three genes selected for their established prognostic and therapeutic significance rather than performing unbiased genome-wide screening: ESR1 (estrogen receptor, defining ER-positive status and guiding hormone therapy), CCND1 (11q13 amplification marker defining IntClust 2, targetable by CDK4/6 inhibitors), and FGFR1 (8p12 amplification marker defining IntClust 6, potential FGFR inhibitor target).
...
Table 1B shows the distribution of IntClust groups within PAM50 subtypes. What stands out here is that IntClust groups 2, 3, 6, 7, 8, and 9 are all predominantly classified as Luminal by PAM50, yet as I show in later sections, they have very different survival outcomes. This cross-tabulation provides the first indication that PAM50 Luminal classification masks substantial molecular and prognostic diversity.

## 3.2 Question 1: Gene Expression Analysis

### Rationale for Gene Selection

I focused on three genes with established prognostic and therapeutic significance that highlight gaps in PAM50-based classification:
...
I classified IntClust 2 and 6 as "high-risk" and IntClust 3, 7, and 8 as "low-risk" based on both the original METABRIC survival data and their distinct genomic drivers.
...
**TP53 Mutations:** TP53 mutation frequency differed significantly across IntClust groups (chi-square p = `r format.pval(tp53_pval, digits=3)`) (Figure 2E). High-risk IntClust groups (2 and 9) showed elevated TP53 frequencies, reflecting the association between TP53 loss and genomic instability.
...
The clinical comparison table above shows that standard clinical variables—age, menopausal status, ER status, and treatment received—do not meaningfully distinguish hidden high-risk from standard Luminal patients. This is an important finding: it means clinicians relying on routine clinical features alone would have no way of identifying these high-risk patients without genomic profiling that captures copy number alterations.
...
The molecular drivers plot confirms that while ESR1 expression remains uniformly high across both groups (consistent with their shared Luminal classification), CCND1 and FGFR1 are significantly elevated in the hidden high-risk group. These represent the specific genomic amplifications driving the poor prognosis—and importantly, both have therapeutic implications (CDK4/6 inhibitors for CCND1, FGFR inhibitors for FGFR1) that are not discussed with patients under current PAM50-only classification.
...
METABRIC represents one of the largest, most well-characterised breast cancer cohorts with integrated clinical, genomic, and transcriptomic data. Our analytical approach systematically addressed three mandatory questions before extending to the patient communication research question. We employed appropriate statistical methods throughout, including non-parametric tests for expression comparisons and log-rank tests for survival analysis. All code is documented and reproducible with relative file paths and automated package installation.

## Limitations

Several limitations should be considered. First, METABRIC samples were collected between 1977 and 2005, predating contemporary treatments including CDK4/6 inhibitors and modern chemotherapy regimens. Treatment patterns and outcomes may differ in current clinical practice. Second, this is a retrospective, single-cohort analysis from UK and Canadian centres; prospective validation in independent and more geographically diverse cohorts would strengthen conclusions. Third, IntClust classification requires comprehensive genomic profiling not routinely performed in clinical practice, limiting immediate clinical applicability. Fourth, the analysis cannot establish causal relationships between genomic features and patient outcomes. Fifth, the survival endpoint is overall survival rather than breast cancer-specific survival; some deaths in this long follow-up cohort may be unrelated to cancer, potentially attenuating true differences between risk groups. Sixth, our survival analyses are univariate (Kaplan-Meier with log-rank tests); multivariate Cox regression adjusting for confounders such as age, treatment received, and tumour grade would strengthen the claim that IntClust provides independent prognostic value. Finally, multiple statistical comparisons were performed across sections without formal correction for multiple testing, and results should be interpreted accordingly.
...
This analysis demonstrates that approximately one in five PAM50 Luminal breast cancer patients harbour high-risk molecular features not captured by standard classification. IntClust groups 2, 6, and 9 showed significantly worse survival outcomes despite patients receiving uniform "favourable prognosis" messaging based on their Luminal classification. 

The survival difference of `r round(death_rate_diff, 1)` percentage points between hidden high-risk and standard Luminal patients is clinically meaningful. Standard clinical variables could not identify this high-risk subset, demonstrating that molecular profiling capturing copy number alterations provides additional prognostic information not captured by expression-based classification or routine clinical assessment.

These findings support the case for incorporating comprehensive genomic data into patient discussions about prognosis, so that patients with heterogeneous molecular profiles receive more accurate risk information.
